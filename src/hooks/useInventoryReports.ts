  ...
  const unusedItemsQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data: packagingMaterials, error: packError } = await supabase
        ...
        const unusedItems: InventoryItem[] = [];
        ...
        if (fetchRawMaterials) {
          ...
          const usedRawIds = new Set(
            (semiIngredients || []).map(item => item.raw_material_id)
          );
          
          const usedRawCodes = new Set(
            (productionIngredients || []).map(item => item.raw_material_code)
          );
          ...
          unusedItems.push(...unusedRaw);
        }
        
        // Process packaging materials if needed
        if (fetchPackagingMaterials) {
          const { data: packagingItems, error: packItemsError } = await supabase
          ...
          const usedPackagingCodes = new Set(
            (packagingMaterials || []).map(item => item.packaging_material_code)
          );
          
          // Find unused packaging materials
          const unusedPackaging = (packagingItems || [])
            .filter(item => 
          ...
          unusedItems.push(...unusedPackaging);
        }
  ...
  const stagnantItemsQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const stagnantItems: InventoryItem[] = [];
        ...
        const processCategory = async (
          ...
          for (const item of items || []) {
            ...
            if (lastMovementDate) {
              ...
              // If no movement, assume older than threshold
              daysSince = filter.period + 30;
            }
        ...
        stagnantItems.push(...rawItems, ...packagingItems, ...semiItems, ...finishedItems);
        
        // Sort by days since last movement
        return stagnantItems.sort((a, b) => 
          (b.daysSinceLastMovement || 0) - (a.daysSinceLastMovement || 0)
        );
      } catch (error) {
