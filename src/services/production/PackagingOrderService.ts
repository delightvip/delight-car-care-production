...
import { PackagingOrder, ProductionStatus } from "./ProductionTypes";

class PackagingOrderService {
  private static instance: PackagingOrderService;
  ...
  public static getInstance(): PackagingOrderService {
    if (!PackagingOrderService.instance) {
      PackagingOrderService.instance = new PackagingOrderService();
    }
    return PackagingOrderService.instance;
  }
  ...
  public async updatePackagingOrderStatus(orderId: number, newStatus: ProductionStatus): Promise<boolean> {
    try {
...
export default PackagingOrderService;
