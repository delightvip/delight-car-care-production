...
import { LowStockItemProps } from './LowStockCard';

interface LowStockStatsProps {
  allItems: LowStockItemProps[];
  criticalItems: number;
}

const LowStockStats: React.FC<LowStockStatsProps> = ({ allItems, criticalItems }) => {
  return (
      ...
      <Card className="bg-amber-50 dark:bg-amber-950/20">
        <CardContent className="p-6 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-bold text-amber-900 dark:text-amber-300 mt-1">{allItems.length}</h3>
          </div>
