...
interface PackagingSummaryCardsProps {
  total: number;
  pending: number;
  inProgress: number;
  completed: number;
...
const cardConfigs = [
  {
    key: 'total',
    title: 'إجمالي أوامر التعبئة',
    icon: <ClipboardList className="h-7 w-7" />, 
    color: 'primary',
    description: 'عدد جميع أوامر التعبئة المسجلة',
    className: 'order-1',
  },
  {
    key: 'pending',
  ...
  {
    key: 'totalCost',
    title: 'إجمالي تكلفة التعبئة',
    icon: <ClipboardList className="h-7 w-7" />, 
    color: 'secondary',
    description: 'إجمالي تكلفة جميع أوامر التعبئة',
...
const PackagingSummaryCards: React.FC<PackagingSummaryCardsProps> = ({
  total,
  pending,
  inProgress,
  completed,
...
export default PackagingSummaryCards;
