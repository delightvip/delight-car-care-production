...
const CashDepositForm: React.FC<CashDepositFormProps> = ({ onSuccess }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      await cashManagementService.depositToAccount(
        amount,
        values.accountType,
        values.date,
        values.notes || '',
  ...
  return (
    <Card className="border-0 shadow-none">
      <CardHeader className="px-0 pt-0">
        <CardTitle className="text-lg">إيداع مبلغ في الخزينة</CardTitle>
        <CardDescription>إضافة مبلغ إلى الخزينة النقدية أو الحساب البنكي</CardDescription>
      </CardHeader>
      <CardContent className="px-0">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      رقم الإيصال أو المرجع الخاص بعملية الإيداع (اختياري)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <Button type="submit" className="w-full">إيداع المبلغ</Button>
          </form>
        </Form>
      </CardContent>
    </Card>
